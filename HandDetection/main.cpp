...
bool handDetect(cv::Mat image)//引用元がC言語で書いてるのでここだけC言語
{
	...
	{
		cv::Vec3b *imagePtr = image.ptr<cv::Vec3b>(y);
		for (int x = 0; x < image.cols; x++)
		{
			for (int i = 0; i < 3; i++)
			{
				src->imageData[src->widthStep * y + x * 3 + i] = imagePtr[x][i];
			}
		}
	}
...
int main()
{
	...
	capture >> img;
	CvSize sz = cvSize(img.cols, img.rows);
	IplImage* src = cvCreateImage(sz, 8, 3);
	//IplImage* gray = cvCreateImage(cvSize(270, 270), 8, 1);
	IplImage* gray = cvCreateImage(sz, 8, 1);
		...
		if (cn > 0)
		{
			...
			if (max_area > 1000)
			{
				...
				for (; defects; defects = defects->h_next)
				{
					...
					cvDrawContours(src, maxitem, CV_RGB(255, 0, 0), CV_RGB(128, 128, 128), 0, CV_FILLED, 8);
